...
public class OwnerAggregation {
  ...
  public static QueryableDataSet apply(QueryableDataSet input, List<Integer> groups, List<OneDBExpression> aggs, List<FieldType> types, OwnerService ownerService, TaskInfoProto taskInfo) {
    ...
    for (OneDBExpression exp : aggs) {
      aggFunctions.add(OwnerAggregteFunctions.getAggregateFunc(exp, ownerService.getOwnerSideRpc(), ownerService.getThreadPool(), taskInfo));
    ...
    return applyAggregateFunctions(input, aggregator, receiverId == ownerService.getOwnerSideRpc().ownParty().getPartyId());
  }
