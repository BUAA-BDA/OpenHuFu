...
public class OwnerAggregateFunctions {
  ...
  public static class GMWSum implements AggregateFunction<Row, Comparable> {
    ...
    public Comparable aggregate() {
      try {
        List<byte[]> localShares = (List<byte[]>) gmw.run(taskInfo.getTaskId(), taskInfo.getPartiesList(), ImmutableList.of(OpenHuFuCodec.encodeInt(sum)), CircuitType.ADD_32.getId());
