...
public class FQSort extends Sort implements FQRel {

  public FQSort(RelOptCluster cluster, RelTraitSet traitSet, RelNode child, RelCollation collation) {
    super(cluster, traitSet, child, collation, null, null);
    assert getConvention() == FQRel.CONVENTION;
  ...
  public Sort copy(RelTraitSet traitSet, RelNode input, RelCollation newCollation, RexNode offset, RexNode fetch) {
    return new FQSort(getCluster(), traitSet, input, collation);
  }
