...
public class FQTable extends AbstractQueryableTable implements TranslatableTable {
  private static final Logger LOG = LoggerFactory.getLogger(FQTable.class);

  final FQSchemaManager schemaManager;
  final RelProtoDataType protoRowType;
  final FQTableSchema tableInfo;

  FQTable(String tableName, FQSchemaManager schemaManager, Schema schema,
      RelProtoDataType protoRowType) {
    super(Object[].class);
    this.schemaManager = schemaManager;
    this.protoRowType = protoRowType;
    this.tableInfo = new FQTableSchema(tableName, schema);
  }

  public static Table create(FQSchemaManager schemaManager, GlobalTableConfig tableMeta) {
    ...
    if (localInfos.size() > 0) {
      ...
      table = new FQTable(tableName, schemaManager, standard.getSchema(), dataType);
      for (Pair<OwnerClient, TableSchema> pair : localInfos) {
  ...
  public static Table create(FQSchemaManager schemaManager, String tableName, Map operand,
      RelProtoDataType protoRowType) {
    ...
    for (Map<String, Object> fed : components) {
      ...
      if (table == null) {
        ...
        table = new FQTable(tableName, schemaManager, schema, dataType);
  ...
  public Enumerable<Object> query(long planId) {
    return new AbstractEnumerable<Object>() {
      ...
      public Enumerator<Object> enumerator() {
        if (enumerator == null) {
          this.enumerator = new FQEnumerator(schemaManager, planId);
        } else {
  ...
  public <T> Queryable<T> asQueryable(QueryProvider queryProvider, SchemaPlus schema,
      String tableName) {
    return new FQQueryable<>(queryProvider, schema, this, tableName);
  }

  @Override
  public RelNode toRel(ToRelContext context, RelOptTable relOptTable) {
    final RelOptCluster cluster = context.getCluster();
    return new FQTableScan(cluster, cluster.traitSetOf(FQRel.CONVENTION), relOptTable, this,
        null);
  }

  public FQTableSchema getTableSchema() {
    return tableInfo;
  }
  ...
  protected FQSchemaManager getRootSchema() {
    return schemaManager;
  }

  public static class FQQueryable<T> extends AbstractTableQueryable<T> {
    public FQQueryable(QueryProvider queryProvider, SchemaPlus schema, FQTable table,
        String tableName) {
      super(queryProvider, schema, table, tableName);
    }

    private FQTable getTable() {
      return (FQTable) table;
    }
