...
public class FQToEnumerableConverter extends ConverterImpl implements EnumerableRel {
  private static final Logger LOG = LoggerFactory.getLogger(FQToEnumerableConverter.class);

  public FQToEnumerableConverter(RelOptCluster cluster, RelTraitSet traits, RelNode input) {
    super(cluster, ConventionTraitDef.INSTANCE, traits, input);
  }

  @Override
  public RelNode copy(RelTraitSet traitSet, List<RelNode> inputs) {
    return new FQToEnumerableConverter(getCluster(), traitSet, sole(inputs));
  }
  ...
  public Result implement(EnumerableRelImplementor implementor, Prefer pref) {
    final FQRel.Implementor oImplementor = new FQRel.Implementor();
    try {
      oImplementor.visitChild((FQRel) getInput());
    } catch (Exception e) {
      e.printStackTrace();
    }
  ...
  public Result implement(EnumerableRelImplementor implementor, FQRel.Implementor oImplementor,
      Prefer pref) {
