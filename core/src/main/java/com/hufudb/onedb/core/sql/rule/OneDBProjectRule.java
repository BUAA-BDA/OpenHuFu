...
public class OneDBProjectRule extends ConverterRule {
  ...
  public RelNode convert(RelNode rel) {
    ...
    final RelNode child = convert(project.getInput(), project.getInput().getTraitSet().replace(OneDBRel.CONVENTION));
    final RelTraitSet traitSet = child.getCluster().traitSet().replace(OneDBRel.CONVENTION);
    return new OneDBProject(child.getCluster(), traitSet, child, project.getProjects(),
        project.getRowType());
  }
}
