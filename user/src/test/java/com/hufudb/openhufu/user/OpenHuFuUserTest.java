...
public class OpenHuFuUserTest {
  ...
  public void testProject() throws SQLException {
    try (Statement stmt = openHuFuUser.createStatement()) {
      ResultSet rs = stmt.executeQuery("select l_shipmode, " +
          "l_extendedprice * (1 - l_discount) * (1 + l_tax), " +
          "l_extendedprice * (1 - l_discount) * (1 + l_tax), " +
          "l_quantity " +
          "from lineitem " +
          "where  l_shipdate <= 755366400 and l_extendedprice * l_tax >= 230");
      List<ColumnType> header =
          Arrays.asList(ColumnType.STRING, ColumnType.DOUBLE, ColumnType.DOUBLE, ColumnType.DOUBLE);
  ...
  public void testCaseWhen() throws SQLException {
    try (Statement stmt = openHuFuUser.createStatement()) {
      ResultSet rs = stmt.executeQuery("select case  when o_orderpriority = '1-URGENT' " +
  ...
  public void testDistinct() throws SQLException {
    try (Statement stmt = openHuFuUser.createStatement()) {
      ResultSet rs = stmt.executeQuery("select distinct l_orderkey " +
          "from  lineitem where   " +
  ...
  public void testAggregate() throws SQLException {
    try (Statement stmt = openHuFuUser.createStatement()) {
      ResultSet rs = stmt.executeQuery("select  sum(l_quantity),  sum(l_extendedprice),  " +
          ...
          "count(*) from  lineitem");
      List<ColumnType> header =
          Arrays.asList(ColumnType.DOUBLE, ColumnType.DOUBLE, ColumnType.DOUBLE,
  ...
  public void testGroupBy() throws SQLException {
    try (Statement stmt = openHuFuUser.createStatement()) {
      ...
      List<ColumnType> header =
          Arrays.asList(ColumnType.STRING, ColumnType.STRING, ColumnType.DOUBLE,
  ...
  @Test
  public void testSortLimit() throws SQLException {
    try (Statement stmt = openHuFuUser.createStatement()) {
      ResultSet rs = stmt.executeQuery("select l_orderkey, l_extendedprice " +
          "from  lineitem " +
  ...
  public void testJoin() throws SQLException {
    try (Statement stmt = openHuFuUser.createStatement()) {
      ResultSet rs = stmt.executeQuery("select  c_name,  c_custkey,  o_orderkey,  " +
